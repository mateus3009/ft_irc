...
InputBuffer::InputBuffer(shared_ptr<SocketConnection> connection)
    : _connection(connection), _buffer(), _position(0)
{
    memset(_buffer, 0, BUFFER_SIZE);
}

static bool isnoprint(int c) { return !::isprint(c); }
...
std::vector<std::string> InputBuffer::read(void)
{
    ...
    while (index < _position)
    {
        ...
        _buffer[index] = '\0';
        std::string piece = std::string(_buffer + start);
        piece.erase(std::remove_if(piece.begin(),piece.end(), ::isnoprint), piece.end());
        messages.push_back(piece);
    }
...
OutputBuffer::OutputBuffer(shared_ptr<SocketConnection> connection)
